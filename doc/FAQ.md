...
- Uses latest Sumokoin core code
...
- Manually import existing wallet (by copying them to the sumowallet folder)
...
of your first transaction or the current block height from the testnet daemon)
...
All wallet files (`testnet` and `mainnet`) are stored in the main `sumowallet` folder.
...
- Windows: `%USERPROFILE%\Documents\Sumokoin\wallets`
- Mac: `~/.sumokoin/wallets` for the daemon.
- Linux: `~/.sumokoin/wallet`
...
**NB: This feature is for test purposed only - all your SUMO will be stolen if you use it!**
